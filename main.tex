...
\begin{itemize}
    ...
    \item Triagem Inteligente: Implementar componentes de IA para auxiliar o condutor na triagem preliminar do problema, otimizando o preparo do prestador.
    \item Segurança e Validação: Estabelecer um fluxo de cadastro e validação de identidade para condutores e prestadores, mitigando riscos de fraude, informalidade e exposição indevida de dados.
\end{itemize}
...
As alternativas atualmente utilizadas para obtenção de socorro mecânico, como buscas genéricas, contatos telefônicos, indicações informais e cadastros empresariais, não garantem disponibilidade em tempo real, validação do prestador, compatibilidade com o tipo de veículo ou estimativa de proximidade. Embora atividades de manutenção e reparação mecânica possam ser identificadas formalmente por classificações como a CNAE 4520-0/01, esses registros não resolvem o problema operacional do atendimento emergencial, pois não oferecem um fluxo centralizado de solicitação, triagem e pareamento entre condutor e prestador \cite{concla_ibge_sd, brasil_sd}.

Nesse contexto, o HelpMechanic justifica-se por propor uma plataforma que centraliza a solicitação de socorro mecânico, realiza triagem preliminar assistida por inteligência artificial, aplica pareamento por localização e especialidade e prevê validação de condutores e prestadores. A proposta beneficia condutores, prestadores de serviço e o fluxo urbano-logístico local, ao mesmo tempo em que contribui academicamente para a área de Análise e Desenvolvimento de Sistemas por especificar regras de negócio, requisitos funcionais, requisitos não funcionais e critérios de segurança para uma aplicação móvel baseada em geolocalização. A escolha por tecnologias com foco em desempenho e segurança de tipos reforça a adequação técnica da solução para cenários que exigem comunicação rápida e confiável \cite{trpcteam_sd, honoteam_sd}.
...
O tempo de resposta é vital para sistemas de emergência viária. A adoção de \textit{Edge Computing} permite processamento descentralizado e proximidade com o usuário, mitigando problemas de latência \cite{fama2018}. A utilização conjunta do \textit{runtime} Bun — um conjunto de ferramentas JavaScript/TypeScript que inclui runtime, gerenciador de pacotes, executor de testes e empacotador — e do \textit{framework} Hono — projetado para aplicações web leves e compatíveis com ambientes de borda — consolida uma infraestrutura adequada para minimizar atrasos na comunicação entre condutores e socorristas em situações críticas \cite{bun_sd, honoteam_sd}.
...
\textbf{ID} & \textbf{Nome da Regra} & \textbf{Descrição} & \textbf{Atores} \\ \hline
\endhead
RN001 & Condutor (Cliente) & O condutor deve obrigatoriamente cadastrar seu perfil e seus veículos, leves ou pesados, para utilizar o fluxo de solicitação de socorro. & Condutor \\ \hline
